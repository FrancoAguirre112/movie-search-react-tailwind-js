      ...
      {movies.map((movie) => {
        ...
        return (
          <Link
            ...
            key={movie.id}
            className="bg-cover rounded-lg border-2 border-gray-400 w-auto h-[18rem] lg:h-[30rem] md:hover:scale-105 transition-transform"
          >
            <div className="grid grid-rows-[70%_30%] rounded-lg h-full bg-gradient-to-t from-black">
              <div
                onClick={() => handleFavorites(movie)}
                className={`${
                  isFav ? "text-[#F8C400]" : "text-white"
                } flex items-start justify-end ${isAuthenticated ? "" : "opacity-0"}`}
              >
