...
import noPosterFound from '../assets/images/noPosterFound.png';
  ...
  return (
    <ul className="grid grid-cols-2 gap-5 md:grid-cols-3">
      {movies.map((movie) => {
        ...
        const posterUrl = movie.poster === 'N/A' ? noPosterFound : movie.poster;
            ...
            <div className="flex justify-center rounded-lg h-full bg-gradient-to-t from-black">
              ...
              {isAuthenticated && (
                <button
                  ...
                  className={`${isFav ? 'text-[#F8C400]' : 'text-white'} right-0 absolute z-50`}
                >
...
function NoMovieResults() {
  return <p>No movies were found for the title you specified.</p>;
}
